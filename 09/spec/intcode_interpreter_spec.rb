...
RSpec.describe IntcodeInterpreter do
  ...
  describe '#execute' do
    ...
    context 'opcodes' do
      ...
      context 'with opcode 09 (adjust relative base)' do
        it 'adds the parameter to the relative base' do
          interpreter = IntcodeInterpreter.new([204, 0, 109, 1, 204, 0])

          interpreter.execute
          first_output = interpreter.output.deq
          second_output = interpreter.output.deq

          expect(first_output).to eq(204)
          expect(second_output).to eq(0)
        end
      end
    end

    context 'with immediate parameter modes' do
    ...
    context 'with relative parameter modes' do
      it 'treats the corresponding parameter as an address offset from a relative base' do
        interpreter = IntcodeInterpreter.new([109, 1, 204, -1])
        interpreter.execute

        expect(interpreter.output.deq).to eq(109)
      end

      context 'with opcode 3 (input to destination)' do
        it 'reads from its input queue and writes it to the relative index' do
          interpreter = IntcodeInterpreter.new([109, 1, 203, -1])
          interpreter.input.enq(777)
          expect(interpreter.execute).to    eq([777, 1, 203, -1])
        end
      end
    end
